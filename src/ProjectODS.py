...
def getLabel(onto : Ontology, class1):
    return ''.join(onto.get_id_label(class1))
def main():
    """
        ...
        for ontoName1, ontoName2 in t.toBeMatchedOntologies:
            ...
            if onto1 and onto2:
                ...
                for class1 in tqdm(onto1.get_classes(), desc = f'computing similarities for {ontoName1} X {ontoName2}'):
                    for class2 in tqdm(onto2.get_classes()):
                        similarity = candidate_concept_sim(getLabel(onto1, class1), getLabel(onto2, class2))
                        #if similarity > 0.2:
    ...
    if configODS.get('verbalizeAvailableTriples'):
        for file_path in os.listdir(configODS.get('triplesPath')):
            if file_path.endswith('.json'):
                ontoName1 = file_path.split('_')[-1].split('.')[0]
                onto1 = ontos.get(ontoName1)
                getL = lambda c : getLabel(onto1, c)
                ...
                generatePrompt.verbaliseFile(tripleFilePath, tripleVerbalizedFilePath, getL)
